<?php
/**
 * view.php - view loader for framework template for Digital Orlando
 */
...
class View extends BuildHTML {
	...
	echo '<link id="pageStyle" href="'.$dir.$file.'" rel="stylesheet" type="'.$type.'" media="'.$media.'" />';
	}
	...
	//changed to output meta subject as well as title and description
	if ($key == 'title' || $key == 'description'|| $key == 'meta_subject') { 
	echo $val;
	}
	//added to hide meta_id
	else if ($key == 'meta_id') {
	echo '';
	}
	//added to output "Created by [meta creator]"
	else if ($key == 'meta_creator') {
	echo 'Created by: ', $val;
	}
	//amended to output Source rather than source. 
	else {
	echo ucfirst(str_replace('meta_', '', $key)).': ',$val;
	...
	$input_attributes3 = array("type"=>"submit","value"=>"Search this site");
	...
	$vertul_attributes = array("id" =>'menu');
	$vertul_start = BuildHTML::start_element(self::$ul, $vertul_attributes);
	$ul_end = BuildHTML::end_element(self::$ul);
	$li_start = BuildHTML::start_element(self::$li);
	$li_end = BuildHTML::end_element(self::$li);
	//$link_attributes = array("href"=>'#');
	...
	//output 'Menu' removed for better readability 
	echo $vertul_start;
	echo $li_start.'<br/>'.$li_end;
	} 
	else echo $ul_start;
	...
	//link output abstracted
	$link_attributes = array("href"=>'?node='.$parent_link,"title"=>$parent_desc);
	$link_start = BuildHTML::start_element(self::$link, $link_attributes);
	$link_end = BuildHTML::end_element(self::$link);
	echo '<li id="'.$parent_id.'">',$link_start.$parent_name.$link_end;
	if (is_array($val)) {
	echo $ul_start;
	...
	$link_attributes = array("href"=>'?node='.$child_link,"title"=>$child_desc);
	$link_start = BuildHTML::start_element(self::$link, $link_attributes);
	$link_end = BuildHTML::end_element(self::$link);
	echo '<li id="'.$child_id.'">',$link_start.$child_name.$link_end.$li_end;
	}
	}
	echo $ul_end.$li_end;
	}
	...
	echo $related_start;
	echo $ul_start;
	echo $li_start.'Related Links:'.$li_end;
	...
	//draw related links for content
	...
	//changed to dynamically output "Related [the opposite of whatever content type is currently displayed]".
	echo $li_start.'<a href="?node='.$key.'&id='.$val.'" title="View related">Related '.str_replace('content/','',$key).'</a>'.$li_end;
    //added to facilitate access to parallel facsimile image/text
    echo $li_start.'<a href="?node=content&id='.$val.'" title="View parallel text/image of this page">View parallel text/image</a>'.$li_end;
	...
	function draw_header($main_menu) {
		...
		$logo_attributes = array("src"=>DESIGN_IMAGES_DIR.$logo, "alt"=>"Digital Orlando default logo", "title"=>"Digital Orlando", "class"=>"logo");
		...
		//added feature to turn logo into link to home page/node 1
		$link_attributes = array("href"=>'?node=content/text&id=1');
	    $link_start = BuildHTML::start_element(self::$link, $link_attributes);
	    $link_end = BuildHTML::end_element(self::$link);
		echo $header_start;
		echo $link_start.'<div id="header_logo" class="grid_3">'.$logo_start.'</div>'.$link_end;
